...
$(document).ready(function() {  
	...
	$(selector).each(function(index) {
		if($(this).prop('dataset').notoc !== undefined) return;
		switch( $(this).prop('tagName').toLowerCase() ) {
			...
			case "h3" :
				...
				if (i3 === 1) {
					...
					toc.children().eq(i2-1).append(current_ul);
				} else {
					current_ul = toc.children().eq(i2-1).children().last();
				}
				break;
			case "h4" :
				++i4;
				...
				if (i4 === 1) {
					...
					toc.children().eq(i2-1).children().last().children().eq(i3-1).append(current_ul);					
				} else {
					current_ul = toc.children().eq(i2-1).children().last().children().eq(i3-1).children().last();					
				}
				break;
			case "h5" :
				++i5;
				...
				if (i5 === 1) {
					...
					toc.children().eq(i2-1).children().last().children().eq(i3-1).children().last().children().eq(i4-1).append(current_ul);					
				} else {
					current_ul = toc.children().eq(i2-1).children().last().children().eq(i3-1).children().last().children().eq(i4-1).children().last();										
				}
				break;
			case "h6" :
				++i6;
				section = i2 + "." + i3 + "." + i4 + "." + i5 + "." + i6;
				if (i6 === 1) {
					// We have to add <ul> to the top...
					current_ul = $('<ul class="toc toclevel5"></ul>');
					toc.children().eq(i2-1).children().last().children().eq(i3-1).children().last().children().eq(i4-1).children().last().children().eq(i5-1).append(current_ul);					
				} else {
					current_ul = toc.children().eq(i2-1).children().last().children().eq(i3-1).children().last().children().eq(i4-1).children().last().children().eq(i5-1).children().last();										
				}
				break;
		}
		id = "id_" + section;
		...
		new_tocnumber.attr('class','tocnumber').append(section);
		// See if the corresponding header has a tochidden flag; this has to be copied to the numbering for later processing!
		...
		$(this).prepend($("<span></span>").append(section).attr("id", id));
	})
